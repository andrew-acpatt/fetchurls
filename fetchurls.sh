...
fetchSiteUrls() {
  ...
  | grep '^--' \
  | awk '{ print $3 }' \
  | grep -E -v '\.(css|js|map|xml|png|gif|jpg|JPG|PDF|doc|docx|bmp|txt|pdf|svg|jpeg|JPEG|PNG|xls|xlsx|xlsm|DOCX|ppsx|PPSX|ppt|PPT|zip|ZIP|mp3|MP3|ashx|ASHX|eot|ttf|woff|woff2)(\?.*)?$' \
